...
export class SolanaTransport implements ITransport<SolanaWalletAdapterWallet> {
    ...
    async createGameAccount(
        ...
        if (tokenMintKey == NATIVE_MINT) {
            ...
            stakeAccountKey = pda;
            console.info(
                "Game uses SOL as token, use PDA as stake account:",
                stakeAccountKey,
            );
        } else {
            // For SPL game, use dedicated stake account
            const { tokenAccount: stakeAccount, ixs: createStakeAccountIxs } =
            ...
            console.info(
                "Game uses SPL as token, use dedicated stake account:",
                stakeAccountKey,
            );
        }
    ...
    async join(
        ...
        console.debug(
            "Batched RPC calls took %s milliseconds",
            new Date().getTime() - d.getTime(),
        );

        const profileKey0 =
            playerProfile !== undefined ? address(playerProfile?.addr) : undefined;
        ...
        if (isWsol) {
            ...
            const { ixs: createTempAccountIxs, tokenAccount: tokenAccount } =
                await this._prepareCreateTokenAccount(payer, mintKey);
    ...
    async _prepareCreatePlayerProfile(
        ...
        Result<
            { ixs: IInstruction[]; profileKey: Address },
            CreatePlayerProfileError
        >
    > {
        let ixs = [];
        ...
        const pfpKey = !pfp
            ...
            : address(pfp);
        const createProfile = instruction.createPlayerProfile(
            payer.address,
            profileKey,
            nick,
            pfpKey,
        ...
        console.info(
            "Transaction Instruction[CreatePlayerProfile]:",
            createProfile,
        );
    ...
    async createPlayerProfile(
        ...
        const payer = this.useTransactionSendingSigner(wallet);
        let ixs: IInstruction[] = [];
        const createPlayerProfile = await this._prepareCreatePlayerProfile(
            payer,
            params,
        );
    ...
    async getGameBundle(addr: string): Promise<GameBundle | undefined> {
        ...
        const [metadataKey] = await getProgramDerivedAddress({
            ...
            seeds: [
                "metadata",
                getBase58Encoder().encode(METAPLEX_PROGRAM_ID),
                getBase58Encoder().encode(mintKey),
            ],
        });

        const metadataAccountData =
            await this._getFinializedBase64AccountData(metadataKey);
    ...
    async listPlayerProfiles(
        addrs: string[],
    ): Promise<Array<PlayerProfile | undefined>> {
        ...
        for (let i = 0; i < addrs.length; i += 100) {
            ...
            const keys = await Promise.all(
                addrsChunk.map((addr) => this._getPlayerProfileAddress(addr)),
            );
            const states = await this._getMultiPlayerStates(keys);
            results.push(
                ...states.map((state, j) => state?.generalize(addrsChunk[j])),
            );
        }
        return results;
    }
    ...
    async listTokens(rawMintAddrs: string[]): Promise<Token[]> {
        ...
        let tokens = await Promise.all(
            rawMintAddrs.map((a) => this._getAssetAsToken(address(a))),
        );

        return tokens.filter((t): t is Token => t !== undefined);
    }

    async __fetchAllToken(
        walletAddr: Address,
        mintAddrs: Address[],
    ): Promise<MaybeAccount<SPL.Token, string>[]> {
     ...
     * List tokens.
     */
    async listTokenBalance(
        rawWalletAddr: string,
        rawMintAddrs: string[],
    ...
    async _getMultiGameStates(
        ...
        for (let i = 0; i < accounts.value.length; i++) {
            ...
            if (accountInfo !== null) {
                try {
                    ret.push(
                        GameState.deserialize(base64ToUint8Array(accountInfo.data[0])),
                    );
    ...
    async _getMultiPlayerStates(
        ...
        for (let i = 0; i < accounts.value.length; i++) {
            ...
            if (accountInfo !== null) {
                try {
                    ret.push(
                        PlayerState.deserialize(base64ToUint8Array(accountInfo.data[0])),
                    );
    ...
    // format.
    async _getFinializedBase64AccountData(
        addr: Address,
    ...
    async _getRecipientState(
        recipientKey: Address,
    ): Promise<RecipientState | undefined> {
