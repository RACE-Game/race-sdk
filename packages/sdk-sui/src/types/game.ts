...
export const GameAccountParser: Parser<IGameAccount, typeof GameSchema> = {
    ...
    transform: (input: typeof GameSchema.$inferType): IGameAccount => {
        ...
        return {
            ...
            checkpointOnChain:
                input.checkpointOnChain.length > 0
                    ? CheckpointOnChain.fromRaw(Uint8Array.from(input.checkpointOnChain))
                    : undefined,
            entryLock: ENTRY_LOCKS[input.entryLock],
            bonuses: Array.from(input.bonuses).map(bonus => BonusParser.transform(bonus)),
            balances: Array.from(input.balances).map(balance => PlayerBalanceParser.transform(balance)),
        }
    },
}
